module Refinery
  module Core
    class Engine < ::Rails::Engine
      ...
      class << self
        # Performs the Refinery inclusion process which extends the currently
        # loaded Rails applications with Refinery's controllers and helpers.
        # The process is wrapped by a before_inclusion and after_inclusion step
        # that calls procs registered by the Refinery::Engine#before_inclusion
        # and Refinery::Engine#after_inclusion class methods
        def refinery_inclusion!
          before_inclusion_procs.each(&:call).tap do |c|
            c.clear if Rails.application.config.cache_classes
          end

          Refinery.include_once(
            ::ApplicationController, Refinery::ApplicationController
          )
          ...
          # Register all decorators from app/decorators/ and
          # registered plugins' paths.
          Decorators.register!(
            Rails.root, Refinery::Plugins.registered.pathnames
          )
        end
      end
      ...
      config.to_prepare do
        ActionView::Base.field_error_proc = proc do |html_tag, _instance|
          ActionController::Base.helpers.content_tag(
            :span, html_tag, class: 'fieldWithErrors'
          )
        end
      end
      ...
      before_inclusion do
        Refinery::Plugin.register do |plugin|
          ...
          plugin.menu_match = %r{refinery\/(refinery_)?core$}
        end
        ...
        Refinery::Plugin.register do |plugin|
          ...
          plugin.hide_from_menu = true
          plugin.always_allow_access = true
          plugin.menu_match = %r{refinery\/(refinery_)?dialogs}
        end
      end

      initializer 'refinery.routes' do
      ...
      initializer 'refinery.params.filter' do |app|
        app.config.filter_parameters += %i[password password_confirmation]
      end
